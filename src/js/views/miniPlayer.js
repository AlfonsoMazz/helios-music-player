...
let titleEl, artistEl, compactCover, compactTitle, compactArtist;
...
const volumeWaveIconSVG = `<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="currentColor" d="M12 3v18h-2V3zm4 4v10h-2V7zm4 4v2h-2v-2zm-12 0v2H4v-2zM8 5v14H6V5z"/></svg>`;

// --- Lógica de Arrastre ---
let isDragging = false;
let offsetX, offsetY;
function onDragStart(e) { if (e.target.closest('button') || e.target.closest('input[type="range"]')) return; isDragging = true; const rect = miniPlayerContainer.getBoundingClientRect(); offsetX = e.clientX - rect.left; offsetY = e.clientY - rect.top; miniPlayerContainer.style.transition = 'none'; miniPlayerContainer.style.transform = 'none'; window.addEventListener('mousemove', onDragMove, { passive: false }); window.addEventListener('mouseup', onDragEnd, { once: true }); }
function onDragMove(e) { if (!isDragging) return; e.preventDefault(); const newX = e.clientX - offsetX; const newY = e.clientY - offsetY; miniPlayerContainer.style.left = `${newX}px`; miniPlayerContainer.style.top = `${newY}px`; }
function onDragEnd() { isDragging = false; miniPlayerContainer.style.transition = 'opacity 0.3s ease-out, transform 0.3s ease-out'; window.removeEventListener('mousemove', onDragMove); }
...
const resizeObserver = new ResizeObserver(entries => { for (const entry of entries) { const { width, height } = entry.contentRect; miniPlayerContainer.classList.toggle('is-bar-mode', width / height > 1.8); } });

/** --- CAMBIO 1: Función para rellenar la barra de volumen --- */
...
/** Sincroniza la UI de los botones (shuffle/repeat/volumen) */
function syncButtonsUI() {
    if (!appStateRef) return;
    shuffleBtn.classList.toggle('active-icon', appStateRef.isShuffled);
    repeatBtn.classList.remove('active-icon', 'amber-icon');
    if (appStateRef.repeatState === 1) repeatBtn.classList.add('active-icon');
    else if (appStateRef.repeatState === 2) repeatBtn.classList.add('amber-icon');
    
    const isMuted = appStateRef.volume === 0 || appStateRef.isMuted;
    volumeBtn.classList.toggle('opacity-50', isMuted);
}

/** Actualiza toda la información visual del mini-reproductor */
function update() {
    ...
    if (currentTrack) {
        ...
        compactCover.src = currentTrack.cover;
        compactTitle.textContent = currentTrack.title;
        compactArtist.textContent = currentTrack.artist;
    } else {
        ...
        compactCover.src = 'https://placehold.co/80x80/121212/808080?text=...';
        compactTitle.textContent = '...';
        compactArtist.textContent = '...';
    }
    ...
    updateRangeFill(volumeBar); // <-- CAMBIO 2: Llamar a la función de relleno
    syncButtonsUI();
...
export function initMiniPlayer(appState) {
    ...
    compactCover = document.getElementById('mp-cover-compact');
    compactTitle = document.getElementById('mp-title-compact');
    compactArtist = document.getElementById('mp-artist-compact');
    ...
    shuffleBtn.innerHTML = shuffleIconSVG;
    repeatBtn.innerHTML = repeatIconSVG;
    volumeBtn.innerHTML = volumeWaveIconSVG;
    
    // --- Asignar Event Listeners ---
    closeBtn.addEventListener('click', close);
    miniPlayerContainer.addEventListener('mousedown', onDragStart);
    
    playPauseBtn.addEventListener('click', () => appState.playerControls?.togglePlayPause());
    nextBtn.addEventListener('click', () => appState.playerControls?.playNext());
    prevBtn.addEventListener('click', () => appState.playerControls?.playPrev());
    shuffleBtn.addEventListener('click', () => appState.playerControls?.toggleShuffle());
    repeatBtn.addEventListener('click', () => appState.playerControls?.cycleRepeatState());
    volumeBtn.addEventListener('click', () => appState.playerControls?.toggleMute());
    
    // CAMBIO 3: Actualizar el relleno de la barra de volumen en tiempo real
    ...
    // Listener para el 'seeking' en la barra de progreso
    ...
    updateRangeFill(volumeBar); // <-- Relleno inicial al cargar
